...
class Manage_school(object):
    ...
    def __init__(self):
        if os.path.exists(settings.school_db_file + ".dat"):  # shelve会生成三个文件，其中有.dat结尾
            print(os.path.exists(settings.school_db_file))
            self.school_db = shelve.open(settings.school_db_file + ".dat")  # 打开学校数据库文件
            self.run_manage()  # 运行管理视图
            self.school_db.close()
        else:
            print("\33[31;1m system ifo: init database\33[0m")
    ...
    def initialize_school(self):
        """实例化两个学校北京、上海"""
        self.school_db = shelve.open(settings.school_db_file+".dat")
    ...
    def run_manage(self):
        ...
        while True:
            for key in self.school_db:
                print("学校名称:", key)
            choice_school = input("\33[34;0m 输入选择管理的学校名: \33[0m").strip()
            if choice_school in self.school_db:
                ...
                while True:
                    ...
                    user_func = input("""\033[34;0m 输入要操作的命令： \033[0m""").strip()
    ...
    def add_class(self):
        class_name = input('''\033[34;0m输入要添加的班级名称：\033[0m''').strip()
    ...
    def add_teacher(self):
        ...
        if teacher_class in self.school_obj.school_class:  # 判断班级是否存在
            class_obj = self.school_obj.school_class[teacher_class]  # 获取班级对应的实例
            # print(class_obj)
            # print(self.school_obj)
            # print(self.school_obj.school_class)
            if teacher_name not in self.school_obj.school_teacher:  # 判断招聘的老师是否存在，不存在就建立，存在更新
                print(teacher_name)
                print(self.school_obj.school_teacher)
                self.school_obj.create_teacher(teacher_name, teacher_salary, teacher_class, class_obj)
                print("\33[32;1m讲师招聘成功\33[0m")
            else:
                self.school_obj.update_teacher(teacher_name, teacher_class, class_obj)
                print("\33[32;1m讲师信息更新完成\33[0m")
            self.school_db.update({self.choice_school: self.school_obj})
        else:
            print("\33[31;1m系统错误：关联班级不存在\33[0m")
...
class Manage_teacher(object):
    ...
    def run_manage(self):
        ...
        if choice_school in self.school_db:
            self.choice_school = choice_school
            self.school_obj = self.school_db[choice_school]
            teacher_name = input('''\033[34;0m输入登陆讲师的姓名：\033[0m''').strip()
            while True:
